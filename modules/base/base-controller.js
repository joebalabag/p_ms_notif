const { pascalToTitleCase, slugifyWithDateTime, joinStrings } = require('../../helper/functions/string');//require('../../helper/functions/string');
...
class BaseController {
	...
	async update(req, res) {
		...
		if (Array.isArray(req.body)) {
			payload = req.body.map((item) => ({
				// UpdatedDateTime: convertToSqlDateTime(new Date(), new Date()),
				//UpdatedBy: user,
				...item,
			}));
		} else {
	...
	async myUserLogging(req, result_payload, res) {
		...
		//romer edit
		if (req_payload.CreatedBy != undefined) {
			mycreatedby = req_payload.CreatedBy;
		}
		if (req_payload.UpdatedBy != undefined) {
			mycreatedby = req_payload.UpdatedBy;
		}
		if (req_payload.PostedBy != undefined) {
			mycreatedby = req_payload.PostedBy;
		}
