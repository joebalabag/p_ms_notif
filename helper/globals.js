...
const globals = () => {
  global.formatDate = (date) => {
    ...
    return `${("0" + (today.getUTCMonth() + 1)).slice(-2)}-${(
      "0" + today.getUTCDate()
    ).slice(-2)}-${today.getUTCFullYear()}`;
  };

  global.formatTime = (date) => {
    const today = new Date(date);
    return `${("0" + (today.getUTCHours() % 12 || 12)).slice(-2)}:${(
      "0" + today.getUTCMinutes()
    ).slice(-2)}:${("0" + today.getUTCSeconds()).slice(-2)}${
      today.getUTCHours() < 12 ? "AM" : "PM"
    }`;
  };

  global.formatDateTime = (date) => {
    const today = new Date(date);
    let md = `${("0" + (today.getUTCMonth() + 1)).slice(-2)}-${(
      "0" + today.getUTCDate()
    ).slice(-2)}-${today.getUTCFullYear()}`;
    let mt = `${("0" + (today.getUTCHours() % 12 || 12)).slice(-2)}:${(
      "0" + today.getUTCMinutes()
    ).slice(-2)}:${("0" + today.getUTCSeconds()).slice(-2)}${
      today.getUTCHours() < 12 ? "AM" : "PM"
    }`;
  ...
  global.convertToSqlDateTime = (date) => {
    ...
    return `${date.getFullYear()}-${("0" + (date.getMonth() + 1)).slice(-2)}-${(
      "0" + date.getDate()
    ).slice(-2)}T${date.toTimeString().split(" ")[0]}`;
  };
  ...
  global.CalculateAgeYMD = (bday) => {
    ...
    if (new Date().getMonth() > new Date(bday).getMonth()) {
      ...
      let data =
        new Date().getMonth() -
        new Date(bday).getMonth() +
        12 * (new Date().getFullYear() - new Date(bday).getFullYear());
...
var oAuth = {
  type: 'OAuth2',
  user: process.env.GMAIL_OA2_User,
  clientId: process.env.GMAIL_OA2_ClientID,
  clientSecret: process.env.GMAIL_OA2_ClientSecret,
  refreshToken: process.env.GMAIL_OA2_RefreshToken,
  accessToken: process.env.GMAIL_OA2_AccessToken,
};
...
global.EmailTransporter = {
  ...
  auth: process.env.GMAIL_AUTH_TYPE == 'OAuth2' ? oAuth : normalAuth,
};

global.EmailSetup = {
  // message: {
  //   from: 'hybrainremote@gmail.com',
  //   //   attachments: [
  //   //     {
  //   //       filename: 'text1.txt',
  //   //       content: 'hello world!'
  ...
  preview: false,
  send: true,
  transport: global.EmailTransporter,
};
//END OF EMAIL SETUP
